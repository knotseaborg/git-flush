...
func (c *LLMClient) Init() {
	c.apiKey = os.Getenv("API_KEY")
	if c.apiKey == "" {
		logger.Panic("API_KEY environment variable not set")
		os.Exit(1)
	}
	c.model = os.Getenv("MODEL")
	if c.model == "" {
		logger.Warn("LLM Model not found, defaulting to gpt-4-mini")
		c.model = "gpt-4-mini"
	}
	c.endpoint = os.Getenv("ENDPOINT")
	if c.endpoint == "" {
		logger.Panic("Endpoint environment variable not set")
	}
}
